...
为了区分不同用户的数据，需要引入鉴权机制。当前课程 Agent 是按“所选课程”创建和复用配置的，并不是每个用户各自持有一个 Agent 对象；每次请求仍会从 JWT 取得当前用户 ID，并把它放入数据库查询和 Agent 运行上下文，保证资料、任务和对话操作只访问该用户的数据。

首先要区分认证与授权：认证确认“你是谁”，授权决定“你能做什么”。本项目已经实现认证，以及基于 `user_id` 和课程归属的数据隔离；目前没有管理员、教师、学生等角色或权限表，因此不应把它描述成完整的角色权限系统。
...
所以即使用户 A 知道用户 B 的任务 ID，也不能直接查询用户 B 的任务。这是“已认证用户只能操作自己资源”的授权边界；它不同于按角色授予不同操作权限的 RBAC 系统。
